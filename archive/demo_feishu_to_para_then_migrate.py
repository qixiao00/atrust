#!/usr/bin/env python3
"""
Two-phase demo for migrating permissions from Feishu to Paeira/Para SSO.

Phase 1: match Feishu users to target users where Feishu username equals target AD_no
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Demo: migrate grants from Feishu users to target users after AD_no matching."
    )
    ...
    parser.add_argument("--source-domain", required=True, help="Feishu user directory domain")
    parser.add_argument("--target-domain", required=True, help="Paeira/Para SSO user directory domain")
    parser.add_argument("--source-field", default="name", help="Feishu field used as the username source. Default: name")
    parser.add_argument(
        "--target-field",
        default="AD_no",
        help="Target user field used to compare with the Feishu username. Default: AD_no",
    )
    parser.add_argument(
        "--confirmed-file",
    ...
    parser.add_argument("--output-dir", default="output-feishu-to-para-demo", help="Directory for CSV reports.")
...
def main() -> int:
    ...
    print("Querying source users (Feishu)...")
